...
在本人长期使用日本二手平台寻找商品的过程中，我需要反复浏览Mercari、Yahoo Auctions、
Rakuten 多个平台，并根据品牌、型号、品相、价格和历史经验进行判断。随着关注平台和卖家增加，人工流程逐渐出现信息分散、重复筛选和重点商品容易遗漏的问题，效率较低且很难持续全天进行。

项目的重点不是自动完成交易，而是探索如何让 AI 进入一个真实、长期运行的个人工作流，并保证结果可解释、可验证和可持续运行。因此,我把人工经验拆解为数据采集、规则筛选、LLM 语义判断、Python 确定性计算、人工确认和反馈闭环,再通过**飞书自定义机器人**把候选商品推送到手机。

> **项目状态**:已真实部署并长期运行,个人使用中,持续优化(V1)。
> **定位说明**:核心目标是探索如何把 AI 嵌入一个真实、长期存在的工作流,
> 而不是“用 AI 赚钱”。它不是能自主决策或自动下单的 Agent——LLM 负责语义判断,
> 程序规则负责预筛和确定性计算,买不买永远由我人工确认;规则、案例库和阈值也都由人工维护,
> 系统不会自我修改判断标准。
...
二手市场持续有新品和老款流通,商品状态、价格和上架节奏变化很快。
真实选品时,我需要反复浏览多个平台,结合长期积累的品牌、款式和品相经验做判断。
这个工作流长期存在,但其中大量时间消耗在重复的信息处理上:

- 每天有上百件新商品上架,大部分是基础款或残次品
- 真正值得拍的可能只有 3-5 件,有时甚至没有
- 我做了两年,知道哪些品牌的哪些款式值得买
- 但每天花在"翻商品"上的时间,占了 80%
- 为了及时看到符合条件的商品,还得频繁刷新

这个项目尝试的不是让模型包办选品,而是把隐性的人工经验变成可执行、可检查的流程:
采集程序获取原始商品,规则先做确定性筛选,LLM 处理品牌、案例和参考价等语义判断,
Python 计算汇率、运费、税费、成本和利润,最后由人确认是否采购并用反馈推动规则迭代。

真实采购价格、运费、汇率和利润等字段会完整保留,因为它们是实际决策约束;
它们服务于工作流,不是这个项目的叙事目标。
...
6. **飞书推送与人工反馈**:系统只推送候选,不自动下单;是否采购由人确认。卡片按钮打开带 HMAC 签名的 HTTPS 反馈链接,经 Caddy 反代到本机 `kendama-feedback.service`(5001 不对公网开放),签名校验通过后写入 `kendama.db.feedback_events`,同时兼容写入 `feedback.db`。
...
- `ai_filter.py`:成本模型、税费、运费、利润阈值、标签分档(`calculate_cost()`/
  `assign_tag()` 里的免税门槛、13% 税率区间都是按剑玉价格带调出来的)
...
模型成本也是长期运行的现实约束——**够用、稳定且可持续**比"最强"重要。
