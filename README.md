...
`feedback.db` 里每条记录都是一份"偏好数据集"。
随着样本积累,我会分三步把它转化成实际的推荐优化:

- **路径 A:自动生成 `personalized_signals.md`(短期)**
  每周跑一次脚本,统计被我标"买入"的商品特征——哪些品牌、哪些价位区间、
  哪些关键词出现频率高,自动写成 `personalized_signals.md`,作为额外 context
  注入 prompt。这条路跟"领域知识结构化"哲学一脉相承——只是把知识来源从
  "自己写"变成"自动从反馈里提"。50 行 Python 能搞定。
- **路径 B:embedding 相似度匹配(中期)**
  把商品标题用 embedding 编码,历史买入商品的 embedding 均值就是我的
  "偏好向量",新商品打分时与之算余弦相似度,和 LLM 评分加权融合。
  需要几百条反馈样本起步。
- **路径 C:在反馈数据上训练轻量分类器(长期)**
  Logistic regression / XGBoost 作为 LLM 之外的二次过滤。
  需要至少千条样本。
- **要警惕的陷阱**:回声室效应。系统越推我买过的款,我就越看不到新机会。
  会保留 ~20% 的探索性推送(epsilon-greedy)。

#### 关注卖家监控

煤炉(Mercari)的优质卖家会持续上架同风格商品,
盯人比盯关键词命中率更高。

- 在 `config.yaml` 加 `watched_sellers` 列表
- `scraper.py` 增加 `scrape_seller_page` 方法,逻辑跟现有关键词爬虫复用
- 这些商品在卡片标题加 `[关注卖家]` 标记,区别于普通推送

#### 价格变动监控

商品挂出来几天后降价是常见信号——卖家急着出,有捡漏空间。

- 新增 SQLite 表 `listings(item_id, price_jpy, first_seen_at, last_seen_at)`
- 每轮扫描后,对已存在的商品比对价格;降价 ≥5% 或 ≥500 日元的,
  标记 `[降价]` 单独推送
- 需要从三个平台的 URL 分别提取商品 ID(各平台 URL 格式不同,要分别处理)
