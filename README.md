...
- **先本地预筛再送 LLM**:泛词搜索一次返回 100+ 商品,大部分品牌不在范围内。
  本地预筛后只剩 15-20 条,显著降低 LLM 调用成本
...
如果用 GPT-4 替代 DeepSeek,API 成本会上升 5-10 倍,
对剑玉这种客单价不高的品类不划算。
这也是我选 DeepSeek 的核心理由——**便宜且够用,比"最强"重要**。
